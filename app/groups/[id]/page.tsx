    ...
    return (
        <div className="space-y-6">
            ...
            <section className="rounded-3xl border border-[#ead7d7] bg-white px-8 py-8 shadow-sm">
                ...
                <div className="mt-4 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        {confirmLeave ? (
                            <div className="flex items-center gap-2">
                                <span className="text-sm text-neutral-600">Leave group?</span>
                                ...
                                <button onClick={() => setConfirmLeave(false)} className="rounded-full border border-neutral-200 px-3 py-1.5 text-xs font-medium text-neutral-600 hover:border-neutral-400">Cancel</button>
                            </div>
                        ) : (
                            <button
                                onClick={() => setConfirmLeave(true)}
                                className="rounded-full border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-500 transition hover:border-red-300 hover:text-red-600"
                            >
                                Leave
                            </button>
                        )}
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_260px]">
                {/* Chat */}
                <div className="flex flex-col rounded-3xl border border-[#ead7d7] bg-white shadow-sm overflow-hidden">
                    <div className="border-b border-neutral-100 px-6 py-4">
                        <h2 className="font-semibold text-neutral-950">Group chat</h2>
                    </div>

                    <div className="flex-1 overflow-y-auto px-6 py-4 space-y-1" style={{ maxHeight: "480px" }}>
                        {messages.length === 0 ? (
                            ...
                            messages.map((msg) => {
                                ...
                                return (
                                    <div
                                        key={msg.id}
                                        className={`group relative flex flex-col gap-0.5 pb-1 ${isMe ? "items-end" : "items-start"}`}
                                    ...
                                    >
                                        ...
                                        {/* Reply-to preview */}
                                        {msg.replyTo && (
                                            <div className={`max-w-xs rounded-xl border-l-4 border-[#8C1515] bg-neutral-100 px-3 py-1.5 text-xs text-neutral-500 ${isMe ? "mr-1" : "ml-1"}`}>
                                        ...
                                        <div className={`flex items-center gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                                            <div className={`max-w-xs rounded-2xl px-4 py-2.5 text-sm ${
                                                isMe
                                                    ...
                                                    : "bg-[#f3e7e7] text-neutral-900 rounded-bl-sm"
                                            }`}>
                                                {msg.text}
                                            </div>
                                            ...
                                            {/* Hover actions */}
                                            {hoveredId === msg.id && (
                                                <div className={`flex items-center gap-1 rounded-full border border-neutral-200 bg-white px-2 py-1 shadow-sm ${isMe ? "flex-row-reverse" : ""}`}>
                                                    {EMOJIS.map((emoji) => (
                                                        <button
                                                            key={emoji}
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs transition ${
                                                                reacted
                                                                    ? "border-[#8C1515] bg-[#f3e7e7] font-semibold text-[#8C1515]"
                    ...
                    {replyingTo && (
                        <div className="flex items-center justify-between border-t border-neutral-100 bg-neutral-50 px-4 py-2">
                            <p className="text-xs text-neutral-500">
                                <span className="font-semibold">Replying to {replyingTo.displayName.split(" ")[0]}:</span>{" "}
                                {replyingTo.text.length > 60 ? replyingTo.text.slice(0, 60) + "…" : replyingTo.text}
                            </p>
                            <button onClick={() => setReplyingTo(null)} className="text-xs text-neutral-400 hover:text-neutral-700">✕</button>
                        </div>
                    )}

                    <form onSubmit={handleSend} className="border-t border-neutral-100 px-4 py-3 flex gap-3">
                        <input
                            ref={inputRef}
                            value={messageText}
                            onChange={(e) => setMessageText(e.target.value)}
                            placeholder={replyingTo ? `Reply to ${replyingTo.displayName.split(" ")[0]}…` : "Message the group…"}
                            className="flex-1 rounded-xl border border-neutral-200 bg-[#faf7f5] px-4 py-2.5 text-sm text-neutral-900 placeholder:text-neutral-400 outline-none transition focus:border-[#8C1515] focus:bg-white"
                        />
                        <button
                            type="submit"
                    ...
                    <div className="space-y-3">
                        {members.map((m) => (
                            <div key={m.uid} className="flex items-center gap-2">
                                <div className="h-8 w-8 shrink-0 rounded-full bg-[#f3e7e7] flex items-center justify-center text-xs font-bold text-[#8C1515]">
                                ...
                                {user?.uid === group.createdBy && m.uid !== user.uid && (
                                    <button
                                        onClick={() => removeMemberFromGroup(groupId, m.uid)}
